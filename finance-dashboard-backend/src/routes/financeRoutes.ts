...
const requireAuth = (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    const decoded = jwt.verify(token, process.env.JWT_SECRET || 'finance_dashboard_dev_secret');
...
const getTransactionQuery = (req: Request) => {
  ...
  if (req.query.startDate || req.query.endDate) {
    ...
    if (req.query.startDate) {
      (filter.date as Record<string, Date>).$gte = new Date(String(req.query.startDate));
    }

    if (req.query.endDate) {
      (filter.date as Record<string, Date>).$lte = new Date(String(req.query.endDate));
    }
  }
...
const buildDashboardSummary = (transactions: any[]) => {
...
const createAuthToken = (user: { id: string | { toString(): string }; email: string; role: string; name: string }) =>
  jwt.sign({ id: String(user.id), email: user.email, role: user.role }, process.env.JWT_SECRET || 'finance_dashboard_dev_secret', { expiresIn: '1d' });
...
router.get('/transactions', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const transactions = await Transaction.find(query).sort({ date: -1 }).limit(500).lean();
...
router.get('/dashboard', requireAuth, async (_req: Request, res: Response) => {
  try {
    const transactions = await Transaction.find({}).sort({ date: -1 }).limit(500).lean();
    return res.json({
...
router.get('/export/csv', requireAuth, async (req: Request, res: Response) => {
  try {
    const columns = (req.query.columns as string | undefined || 'date,amount,type,category,status,user,description')
      .split(',')
    ...
    return res.status(500).json({ message: 'CSV export failed', error: (error as Error).message });
  }
});
